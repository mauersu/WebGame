...
function clickCount(id){
	$.getJSON("http://182.92.226.152/fso_newmanager/open/testStatistics.do?callback=?", {gid: id, type: 2}, function(data){});

}
function shareCount(id){
	$.getJSON("http://182.92.226.152/fso_newmanager/open/testStatistics.do?callback=?", {gid: id, type: 3}, function(data){});
}

function scoreCount(id,s){
	$.getJSON("http://182.92.226.152/fso_newmanager/open/testStatistics.do?callback=?", {gid: id, s:s, type: 4}, function(data){});
}
...
function tarHref(type){
	if(type==1){
		...
		var urls =  "http://mp.weixin.qq.com/s?__biz=MzAwNjEyMDc4Mw==&mid=202564593&idx=3&sn=39c9afbcdd7f6442c68f5399137de125#rd";
	}else {
		var urls =  "http://"+basedns+"/test/list/";
	}
	location.href = urls;
